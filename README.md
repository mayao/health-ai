...
- **唯一真源（开发与部署）**：本仓库根目录为 **`/Users/xmly/Projects/HealthAI`**。所有功能开发、脚本执行、`npm run build`、iOS 归档、远端部署与文档更新，均只在此目录下进行。
- **不再使用**：历史路径 **`MyCodex/Health`**（或 `.../MyCodex/Health`）中的副本**不作为**开发或部署依据；请勿在该目录继续改代码或发版，以免与真源分叉。
