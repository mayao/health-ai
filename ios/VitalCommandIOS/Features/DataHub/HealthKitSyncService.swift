...
final class HealthKitSyncService: @unchecked Sendable {
    ...
    func fetchSyncSamples(daysBack: Int = 90) async throws -> [HealthKitMetricSampleInput] {
        ...
        async let restingHeartRate = dailyAverageSamples(
        ...
        async let walkingHeartRateAverage = dailyAverageSamples(
            identifier: .walkingHeartRateAverage,
            kind: .walkingHeartRateAverage,
            unit: HKUnit.count().unitDivided(by: .minute()),
            daysBack: daysBack,
            transform: { $0 }
        )
        async let heartRateVariability = dailyAverageSamples(
        ...
        async let oxygenSaturation = dailyAverageSamples(
        ...
        async let respiratoryRate = dailyAverageSamples(
    ...
    private func dailyAverageSamples(
        identifier: HKQuantityTypeIdentifier,
        kind: HealthKitMetricKind,
        unit: HKUnit,
        daysBack: Int,
        ...
        return try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<[HealthKitMetricSampleInput], Error>) in
            ...
            query.initialResultsHandler = { _, results, error in
                ...
                results.enumerateStatistics(from: startDate, to: Date()) { [self] stats, _ in
                    ...
                    let value = transform(quantity.doubleValue(for: unit))
                    guard value > 0 else {
                        return
                    }
