...
mkdir -p "$DEPLOY_DIR/deploy/env"
cp deploy/env/*.example "$DEPLOY_DIR/deploy/env/"
cp deploy/PUBLIC_ACCESS.md "$DEPLOY_DIR/deploy/PUBLIC_ACCESS.md"
cp deploy/install-cloudflared-service.sh "$DEPLOY_DIR/deploy/install-cloudflared-service.sh"
cp deploy/install-cloudflared-backup-launchd.sh "$DEPLOY_DIR/deploy/install-cloudflared-backup-launchd.sh"
...
if [ "$MODE" = "docker" ]; then
  ...
  echo "    -e HEALTH_JWT_SECRET=your-secret-at-least-32-chars \\"
  echo "    -e HEALTH_LLM_PROVIDER=openai-compatible \\"
  echo "    -e HEALTH_LLM_BASE_URL=https://api.kimi.com/coding/v1 \\"
  ...
  echo ""
  echo "  # 3. 创建 .env 配置"
  echo "  cp deploy/env/health-primary.env.example .env"
  echo "  # 然后填写 PUBLIC_BASE_URL / HEALTH_JWT_SECRET / HEALTH_LLM_API_KEY"
  echo ""
  echo "  # 4. 安装 Node.js 22+ 并启动"
  echo "  chmod +x scripts/*.sh deploy/install-cloudflared-service.sh"
  echo "  ./scripts/run-health-server.sh"
  echo ""
  echo "  # 5. 按 deploy/PUBLIC_ACCESS.md 配置 Cloudflare Tunnel"
  echo ""
fi
