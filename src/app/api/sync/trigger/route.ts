...
export async function POST(request: Request) {
  try {
    ...
    const body = (await request.json().catch(() => ({}))) as { peer_urls?: string[] };
    const summary = await syncWithAllPeers({
      candidatePeerUrls: Array.isArray(body.peer_urls) ? body.peer_urls : [],
    });
    const status = getSyncStatus();
    return jsonOk({ triggered: true, ...status, ...summary });
  } catch (error) {
