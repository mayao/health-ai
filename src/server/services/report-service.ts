...
import { buildFallbackSummary, buildHealthSummarySourceInput, generateHealthSummaryFromStructuredInsights, resolveHealthSummaryProvider } from "../llm/health-summary-service";
import {
  getLatestSampleTime,
...
/**
 * Build a purely local fallback narrative without calling any LLM.
 * Used when the LLM times out or is unavailable.
 */
export function buildFallbackNarrative(
  ...
  const fallbackOutput = buildFallbackSummary(sourceInput);

  return sanitizeHealthSummary({
    provider: "mock",
    model: "local-fallback",
    prompt: { templateId: "fallback", version: "1.0", systemPrompt: "", userPrompt: "" },
...
/**
 * Fast version of getReportsIndexData that only reads saved snapshots from DB.
 * Never triggers LLM calls — safe for use in the dashboard endpoint.
 */
export async function getReportsIndexDataCached(
  ...
  const monthlyReports = listSavedReportSnapshots(database, userId, "monthly").slice(0, 3);
