...
test("health home data reflects newly synced Apple Health samples", async () => {
  ...
  syncHealthKitSamples(
    {
      samples: [
        ...
        },
        {
          ...
          value: 58,
          unit: "count/min",
          sample_time: "2026-03-11T07:10:00+08:00"
        },
        {
          kind: "heartRateVariability",
          value: 41,
          unit: "ms",
          sample_time: "2026-03-11T07:15:00+08:00"
        },
        {
          kind: "oxygenSaturation",
          value: 98,
          unit: "%",
          sample_time: "2026-03-11T07:18:00+08:00"
        }
      ]
    },
    database
  ...
  assert.equal(appleHealth?.highlight, "已同步 7 条");
  ...
  assert.equal(data.charts.activity.data.at(-1)?.activeEnergy, 560);
  assert.equal(data.charts.activity.data.at(-1)?.restingHeartRate, 58);
  assert.equal(data.charts.activity.data.at(-1)?.heartRateVariability, 41);
  assert.equal(data.charts.activity.data.at(-1)?.oxygenSaturation, 98);
